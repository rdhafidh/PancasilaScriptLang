...
TEST(TestDivNumber, TestDivNumber) {
  using namespace val;
  VariantType var = 25.05;
  auto ret = var.divVariantNumber((int64_t)5);
  EXPECT_EQ(true, isOkTypeReturnVariant(ret));
  EXPECT_EQ(true, var == 5.01);
  ret = var.divVariantNumber((int64_t)-0);
  EXPECT_EQ(true, ret == TypeErrorVariant::ERROR_DIVIDE_BY_ZERO);
  EXPECT_EQ(true, var == 5.01);
  ret = var.divVariantNumber((int64_t)0);
  EXPECT_EQ(true, ret == TypeErrorVariant::ERROR_DIVIDE_BY_ZERO);
  EXPECT_EQ(true, var == 5.01);
...
TEST(TestDivDouble, DivDouble) {
  using namespace val;
  VariantType var = 25.05;
  auto ret = var.divVariantDouble (5.0);
  EXPECT_EQ(true, isOkTypeReturnVariant(ret));
  EXPECT_EQ(true, var == 5.01);
  ret = var.divVariantDouble (-0.0);
  EXPECT_EQ(true, ret == TypeErrorVariant::ERROR_DIVIDE_BY_ZERO);
  EXPECT_EQ(true, var == 5.01);
  ret = var.divVariantDouble (0.0);
  EXPECT_EQ(true, ret == TypeErrorVariant::ERROR_DIVIDE_BY_ZERO);
...
TEST(ArithOperationNumberAbstractionOfVariant, DivNumber) {
  using namespace val;
  VariantType var = 25.05;
  auto ret = var.divVariantNumber(VariantType((int64_t)5));
  EXPECT_EQ(true, isOkTypeReturnVariant(ret));
  EXPECT_EQ(true, var == 5.01);
  ret = var.divVariantNumber(VariantType((int64_t)-0));
  EXPECT_EQ(true, ret == TypeErrorVariant::ERROR_DIVIDE_BY_ZERO);
  EXPECT_EQ(true, var == 5.01);
  ret = var.divVariantNumber(VariantType((int64_t)0));
  EXPECT_EQ(true, ret == TypeErrorVariant::ERROR_DIVIDE_BY_ZERO);
...
TEST(ArrayPrintTest, PrintArray) {
  using namespace val;
  using namespace std::string_literals;
  VariantTypeList var(VariantTypeList() << (bool)false << (int64_t)30 << "from"s
                                        << VariantType(3.43));
  EXPECT_EQ((size_t)4, var.size());
  EXPECT_EQ(true, var.at(0).second.getType() == TypeData::TYPE_IS_BOOL);
  EXPECT_EQ(true, var.at(1).second == (int64_t)30);
...
TEST(NDimArrayTest, test) {
  ...
  VariantTypeNDimArray ndiminit({(VariantTypeList() << "foo"s
                                                    << "bar"s << 3.12),
                                 (VariantTypeList() << "foo"s
                                                    << "bar"s << 3.33)});
  ...
  ndiminit << (VariantTypeList() << "nothings"s
                                 << "nothings"s);
  EXPECT_EQ(true, ndiminit.colSize() == 2);
