...
#ifndef USE_CLANG_BUILTIN_ARITH
...
#else
    int64_t res = 0;
    if (__builtin_saddll_overflow(std::get<int64_t>(vardata), n,
                                  (long long *)&res) != 0)
      return TypeErrorVariant::ERROR_ARITH_OVERFLOW;
...
#else
    int64_t res = 0;
    if (__builtin_ssubll_overflow(std::get<int64_t>(vardata), n,
                                  (long long *)&res) != 0)
      return TypeErrorVariant::ERROR_ARITH_OVERFLOW;
...
#else 
    int64_t res = 0;
    if (__builtin_smulll_overflow(std::get<int64_t>(vardata), n,
                                  (long long *)&res) != 0)
      return TypeErrorVariant::ERROR_ARITH_OVERFLOW;
...
TypeErrorVariant VariantType::addVariantNumber(const VariantType &right) {
  TypeErrorVariant ret = TypeErrorVariant::ERROR_UNSUPPORTED_ADD_ARITH_TYPES;
  switch (right.getType()) {
    case TypeData::TYPE_IS_BOOL:
    case TypeData::TYPE_IS_NUMBER:
      ret = addVariantNumber(std::get<int64_t>(right.getVar()));
      break;
    case TypeData::TYPE_IS_DOUBLE:
      ret = addVariantDouble(std::get<double>(right.getVar()));
      break;
    default:
      break;
  }
...
TypeErrorVariant VariantType::subVariantNumber(const VariantType &right) {
  TypeErrorVariant ret = TypeErrorVariant::ERROR_UNSUPPORTED_ADD_ARITH_TYPES;
  switch (right.getType()) {
    case TypeData::TYPE_IS_BOOL:
    case TypeData::TYPE_IS_NUMBER:
      ret = subVariantNumber(std::get<int64_t>(right.getVar()));
      break;
    case TypeData::TYPE_IS_DOUBLE:
      ret = subVariantDouble(std::get<double>(right.getVar()));
      break;
    default:
      break;
  }
...
TypeErrorVariant VariantType::mulVariantNumber(const VariantType &right) {
  TypeErrorVariant ret = TypeErrorVariant::ERROR_UNSUPPORTED_ADD_ARITH_TYPES;
  switch (right.getType()) {
    case TypeData::TYPE_IS_BOOL:
    case TypeData::TYPE_IS_NUMBER:
      ret = mulVariantNumber(std::get<int64_t>(right.getVar()));
      break;
    case TypeData::TYPE_IS_DOUBLE:
      ret = mulVariantDouble(std::get<double>(right.getVar()));
      break;
    default:
      break;
  }
...
TypeErrorVariant VariantType::divVariantNumber(const VariantType &right) {
  TypeErrorVariant ret = TypeErrorVariant::ERROR_UNSUPPORTED_ADD_ARITH_TYPES;
  switch (right.getType()) {
    case TypeData::TYPE_IS_BOOL:
    case TypeData::TYPE_IS_NUMBER:
      ret = divVariantNumber(std::get<int64_t>(right.getVar()));
      break;
    case TypeData::TYPE_IS_DOUBLE:
      ret = divVariantDouble(std::get<double>(right.getVar()));
      break;
    default:
      break;
  }
...
VariantTypeNDimArray::VariantTypeNDimArray(
    ...
    : maxdim(anotherVar.getNDimVectorVariant().size()),
      ndimvector(anotherVar.getNDimVectorVariant()) {}
...
size_t VariantTypeNDimArray::maxDim() const { return maxdim; }
