...
#if defined(__clang__) && __clang_major__>=6 && !defined(WIN32)
#define USE_CLANG_BUILTIN_ARITH 1
#endif
...
class VariantType {
 public:
  ...
  TypeErrorVariant addVariantNumber(const VariantType &right);
  TypeErrorVariant subVariantNumber(const VariantType &right);
  TypeErrorVariant mulVariantNumber(const VariantType &right);
  TypeErrorVariant divVariantNumber(const VariantType &right); 
